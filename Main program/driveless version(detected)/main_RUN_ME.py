...
def init():
    ...
    visEngine = pm.read_longlong(get_base_addr() + offsets['VisualEnginePointer'])
    print(f'Visual engine: {visEngine:x}')
    
    fakeDatamodel = pm.read_longlong(visEngine + offsets['VisualEngineToDataModel1'])
    print(f'Fake datamodel: {fakeDatamodel:x}')
    
    dataModel = pm.read_longlong(fakeDatamodel + offsets['VisualEngineToDataModel2'])
    print(f'Real datamodel: {dataModel:x}')
    
    wsAddr = pm.read_longlong(dataModel + offsets['Workspace']) #FindFirstChildOfClass(dataModel, 'Workspace')
    print(f'Workspace: {wsAddr:x}')
    
    camAddr = pm.read_longlong(wsAddr + offsets['Camera']) #FindFirstChildOfClass(wsAddr, 'Camera')
...
def setNewAnims():
    ...
    char = pm.read_longlong(hum + offsets['Parent'])
...
def setAnimPack():
    ...
    for anim in animPackInfo['items']:
        if anim['type'] == 'Asset':
            ...
            WriteRobloxString(getAnim(animType)+offsets['AnimationId'], 'http://www.roblox.com/asset/?id='+trueAnim)
            print('Applied\n-------------------------------------------')
    print('COMPLETE!')
